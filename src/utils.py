...
def suitable_transactions(xls_file: list[dict], search_line: str) -> Any:
    ...
    for transaction in xls_file:
        if search_line == transaction["Категория"] or search_line == transaction["Описание"]:
...
def each_card(transactions: list) -> list[dict]:
    ...
    for i in transactions:
        if isinstance(i["Номер карты"], str) and i["Сумма операции"] < 0:
            dict_summ[i["Номер карты"]] = dict_summ.get(i["Номер карты"], 0) + i["Сумма операции"] * -1
            if i["Сумма операции"] * -1 > 100:
                dict_cashback[i["Номер карты"]] = (
                    dict_cashback.get(i["Номер карты"], 0) + (i["Сумма операции"] * -1) / 100
                )
    list_card = []
...
def top_transactions(transactions: list) -> list[dict]:
    ...
    list_transactions = []
    logger.info("Фильтруем список транзакций: добавляем только траты и преобразовываем их в +")
    for i in transactions:
        if i["Сумма операции"] < 0:
            i["Сумма операции"] = i["Сумма операции"] * -1
            list_transactions.append(i)
    max_transactions = []
    logger.info("Добавляем в список max_transactions топ-5 транзакций по сумме операции")
    for item in range(5):
...
def currency(currency_list: list) -> list:
    ...
    for item in currency_list:
        url_usd = f"https://v6.exchangerate-api.com/v6/{api_token_currency}/latest/{item}"
...
def filter_by_date(date: datetime, end_date: datetime, transactions: list[dict]) -> list[dict]:
